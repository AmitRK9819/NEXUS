"""
Data Fusion Engine — Analytics Service
...
IDS Formula: IDS = 0.3*V + 0.3*(1-S) + 0.2*P + 0.2*(1-I)
"""

from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.future import select
from sqlalchemy import func
from app.models.domain import DemographicData, CitizenRequest, PublicBudgetPlan, InfrastructureIndex


async def calculate_region_metrics(session: AsyncSession):
    """Calculate IDS + Misalignment Index for all regions."""

    # Step 1: Complaint volume & avg sentiment per region
    # Use ST_Contains to spatially join complaints to regions
    stmt_vol = select(
    ...
    # Step 2: Fetch all regions with budget totals and infra scores
    stmt_regions = select(
        DemographicData,
        InfrastructureIndex,
    ).outerjoin(
    ...
    for demo, infra in regions_data:
        ...
        # S: Sentiment normalized to 0-1 (original is -1 to 1)
        raw_sentiment = r_metrics["avg_sentiment"]
        S = (raw_sentiment + 1.0) / 2.0  # Map [-1,1] → [0,1]
        S_inv = 1.0 - S

        # P: Normalized population density
        P = demo.population_density / max_pop if max_pop > 0 else 0.0

        # I: Infrastructure composite (0-1 scale)
        if infra:
            I_score = infra.composite_score
        else:
            I_score = 1.0  # Assume perfect if no data
        I_inv = 1.0 - I_score
    ...
    for i, res in enumerate(results):
        ...
        # Critical hotspot: high IDS percentile + low budget percentile
