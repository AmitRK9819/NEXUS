"use client";
// ============================================================
// NEXUS — Stats Panel: Top 5 Hotspots (Member 3: Mapper)
// ============================================================
...
export default function StatsPanel() {
  ...
  return (
    <div className="bg-gray-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-xl">
      {/* Header stats */}
      <h2 className="text-white text-sm font-semibold mb-3">Top Hotspots</h2>
        ...
        <div className="flex flex-col gap-2">
          {top5.map((p, idx) => {
            ...
            return (
              <button
                key={p.id}
                id={`hotspot-item-${p.id}`}
                onClick={() => selectHotspot(isSelected ? null : p.id)}
                aria-pressed={isSelected}
                aria-label={`Hotspot in ${p.region}, severity ${p.severity_score}`}
                className={`
