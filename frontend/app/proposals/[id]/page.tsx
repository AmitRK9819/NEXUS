...
export default function ProposalPage({ params }: PageProps) {
  const { id } = use(params);
  const { getRecommendation, getProposal, saveProposal, recordDecision } = useStore();
  ...
  }, [proposal, rec, generating, handleGenerate]);
    ...
    <>
      <div className="ai-disclosure-banner flex items-center gap-2 px-4 py-2.5 sm:px-6" role="alert">
        ...
        <p className="text-xs font-medium text-amber-800">
          <strong>AI Project Proposal Brief</strong> — all sections are editable by policymakers prior to formal approval and budget disbursement.
        </p>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          ...
          <aside className="lg:col-span-1 space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <dl className="mt-3 space-y-1.5 text-xs">
                <div className="flex justify-between">
                  ...
                  <dd className="font-semibold text-slate-900">{rec.estimatedCost}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-500">Urgency SLA</dt>
                  <dd className="font-semibold text-slate-900">{rec.urgencyDays} days</dd>
                </div>
              </dl>
            </div>
            ...
            {proposal && (
              <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-2 shadow-sm">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-700 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-800 shadow-sm"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-orange-400 bg-orange-50 px-3 py-2 text-xs font-semibold text-orange-800 hover:bg-orange-100"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-100"
                >
            ...
            <button
              ...
              disabled={generating}
              className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-50"
            >
              {generating ? (
          ...
          <main className="lg:col-span-3">
            {generating && (
              <div className="flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-white py-20 text-center shadow-sm">
                ...
                <p className="text-sm font-medium text-slate-700">Synthesizing project brief with Gemini 2.0…</p>
              </div>
            )}
                  ...
                  <section key={key} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    {key === "title" ? (
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-900 focus:border-slate-900 focus:outline-none"
                      />
                    ) : (
                      <textarea
                        value={proposal[key]}
                        ...
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs leading-relaxed text-slate-900 focus:border-slate-900 focus:outline-none"
                      />
                    )}
                  </section>
                ))}
                      ...
                      <div key={i} className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                        <div className="mb-2 flex items-center justify-between gap-2">
                          <input
                            ...
                            className="flex-1 rounded border border-slate-200 px-2 py-1 text-xs font-semibold text-slate-900"
                          />
                          <input
                            type="text"
                            value={phase.duration}
                            onChange={(e) => updateTimeline(i, "duration", e.target.value)}
                            className="w-28 rounded border border-slate-200 px-2 py-1 text-xs text-slate-600"
                          />
                          <button
                            type="button"
